...
| `auto_migrate` | `true` | Live-migrate the instance during host maintenance. Forced off for preemptible instances, which GCE never migrates. Some machine families, including `e2-*`, reject `false` on a non-preemptible instance. |
